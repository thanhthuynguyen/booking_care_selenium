...
//    }
//
//    @AfterMethod // Annotations này đảm bảo code luôn chạy sau mỗi @Test
//    public void cleanUpData() {
//        if (registeredCourseNames.isEmpty()) {
//            return; // Nếu không có khóa nào được đăng ký thì không cần xóa
//        }
//
//        WebDriver driver = DriverFactory.getDriver();
...
//        JavascriptExecutor js = (JavascriptExecutor) driver;
//
//        System.out.println("\n--- [CLEAN UP] Đang dọn dẹp dữ liệu sau khi Test... ---");
//
//        try {
//            driver.get("https://demo2.cybersoft.edu.vn/thongtincanhan");
//            wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[text()='Khóa học']"))).click();
...
//                try {
//                    // XPath tìm nút Hủy dựa vào tên khóa học trong danh sách
//                    By byBtnCancel = By.xpath("//div[contains(@class,'myCourseItem')][.//h6[text()='\" + courseName + \"']]//button[contains(text(),'Hủy khóa học')]");
//
//                    WebElement btnCancel = wait.until(ExpectedConditions.elementToBeClickable(byBtnCancel));
//                    js.executeScript("arguments[0].click();", btnCancel);
//                    System.out.println("✅ Đã hủy thành công khóa học: " + courseName);
//                } catch (Exception e) {
//                    System.out.println("⚠️ Không tìm thấy nút Hủy cho khóa: " + courseName);
//                }
//            }
//        } catch (Exception e) {
//            System.out.println("❌ Lỗi trong quá trình Clean Up: " + e.getMessage());
//        } finally {
//            // Sau khi dọn dẹp xong, xóa danh sách để lần chạy tiếp theo không bị nhầm
//            registeredCourseNames.clear();
//        }
//    }
//}
...
public class Course_05_Register_Multiple_Courses_Authenticated_User_Success extends BaseTest {
    ...
    public void registerMultipleCoursesAuthenticatedUserSuccessfully() {
        ...
        for (int i = 1; i <= 2; i++) {
            ...
            String courseName = wait.until(ExpectedConditions.visibilityOfElementLocated(byDetailTitle)).getText().trim();
            registeredCourseNames.add(courseName);
            LOG.info("Tên khóa học mục tiêu: " + courseName);
            System.out.println("Tên khóa học mục tiêu: " + courseName);
        ...
        if (allMatched) {
            LOG.info("PASS: Tất cả khóa học " + registeredCourseNames + " đều tìm thấy trong My Account.");
            System.out.println("PASS: Tất cả khóa học " + registeredCourseNames + " đều tìm thấy trong My Account.");
        } else {
            LOG.info("FAIL: Có khóa học trong danh sách " + registeredCourseNames + " bị thiếu hoặc sai tên.");
            LOG.info("Danh sách thực tế trong Account: " + actualEnrolledNames);
            System.out.println("FAIL: Có khóa học trong danh sách " + registeredCourseNames + " bị thiếu hoặc sai tên.");
            System.out.println("Danh sách thực tế trong Account: " + actualEnrolledNames);
        }
